...
bt<-boosting.cv(rating~., v=10, data=wine)
bt$confusion
...
wine_rf <- randomForest(as.factor(wine$rating)~.,data=wine,mtry=sqrt(10),importance=TRUE)
wine_rf
plot(wine_rf) 
varImpPlot(wine_rf) 
...
### cv function for rf
cv_funct_rf <-function(fold){
  train <- wine[-fold,] 
  test_x <- wine[fold,]
  test_y <- wine[fold,"rating"]
  wine_rf <- randomForest(as.factor(train$rating)~.,data=train, mtry=sqrt(10),importance=TRUE)
  summary(wine_rf)
  ypred<-predict(wine_rf,newdata=test_x)
  return (table(ypred,test_y))
}
...
library(tree)
set.seed(1)
cv_funct_t <- function(fold){
  train <- wine[-fold,]
  test_x<- wine[fold,]
  test_y<- wine[fold,"rating"]
  tree.wine<-tree(as.factor(train$rating)~.,data=train)
  cv<-cv.tree(tree.wine, FUN=prune.misclass)
  min = 9999999
  minIdx = 0
  for(i in 1:length(cv$dev)){
    if(min>cv$dev[i]) 
      minIdx=i 
      min=cv$dev[i]
  }
  prune.wine<-prune.misclass(tree.wine,best=cv$size[minIdx])
  tree.pred<-predict(prune.wine,newdata=test_x,type="class")
  return(table(tree.pred,test_y))
}
